...
naes = dset.ImageFolder(root="/work/zhangzherui/data/imagenet-a/", transform=test_transform)
...
# weights_dict = torch.load("./FKD_ResNet50_80.1.pth", map_location="cpu")
# weights_dict = torch.load("./FerKD_ResNet50.pt", map_location="cpu")
weights_dict = torch.load("/work/zhangzherui/code/FKD/FKD_nc_4_res50_plain/my_best.pth", map_location="cpu")["state_dict"]
# modified_dict = {key.replace('module.', ''): value for key, value in weights_dict.items() if key.startswith('module.')}
modified_dict = {key.replace('module.', ''): value for key, value in weights_dict.items()}
print(net.load_state_dict(modified_dict, strict=False))
